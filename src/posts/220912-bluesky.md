...
## Avoid writing your own templating engine, if you're in a hurry
...
your own. There's one in C++ I eyed a little forlorn, [inja][inja], which is a
...
## Multiple passes is way easier than one
...
Then, I still tried to keep some efficiency by making only one rendering pass
per file. The basic pass worked like this: loop through the contents looking for
`<bluesky-`, my common opener. For each opener, identify the tag, parse it, push
the parsed text into a string stream.
...
## Recursive page parsing is easier than a stack

In the same vein, because each file is dependent on an arbitrary number of other
files, it is quite difficult to determine which files to parse first. You start
on a file, recognize it requires a template, so you move to parse that. Then you
recognize an include in the template, so now you have to parse the include. This
sort of dependency chain is very hard to determine in situ, so its much better
to just write a class that can determine its own dependencies and parse them on
its own.

## Don't shove all the functionality in one class

It also occurred to me at far too late a point that it was a bad idea to try to
parse a Template as a Page. Even though a Template can contain the same tags as
a Page, the nature of the parsing and use of the page is different, so trying to
shoehorn them into the same class turned out to be a disaster. Once I split the
Template functionality into its own class, it became much easier to write and
use.

Actually, Templates in bluesky can't use the markdown tag, so they're not
equivalent anyway.

## You'll need a development server
...
resource intensive so its a bad idea to just loop and poll for last modified
times. There are system apis to do that without polling. However, they are very
different in different OS's, so its best to use a library if you want a cross
platform solution. I didn't want to use a library, and I didn't want to poll, so
I did something simpler.

Every time a browser makes a request to bluesky, bluesky loops through all the
files in the source directory (_all of them_), and checks if any of the last
modified times are younger than what are stored in a hash map. If a single one
is, the entire site is regenerated. There are 80 files in my source directory
currently. I do not notice any latency in the browser when the entire site
regenerates. It is incredibly fast.
...
(it seems like a simple fix, I'll get to it soon enough).
...
is not because of security, so to do it I'd have to spin off a thread in bluesky
with an endpoint for the javascript to poll to check the last modified time to
result in a refresh. So I've abandoned it. For now, I can refresh after I make a
change. It's not that much work.
...
One thing I can say without a doubt, if you intend to write your own SSG, do not
under any circumstances take it upon yourself to write your own markdown parser.
I never tried, but judging by the code of md4c, the parser I chose to use, it is
incredibly complex. I did not imagine that would be the case, because markdown
is so simple, but md4c has so many lines of code that, with just the 3 basic
necessary files I included for its use in bluesky, Github says my project is 80%
C.
...
## Prefer file agnosticism to depending on file types

When I started writing bluesky I initially hardcoded some parsing requirements:
includes and templates had to be in a directory called `_imports`. I realized
this was an arbitrary limitation and changed the path attribute in templates to
be a relative path from the calling file. This allows templates to be located
anywhere. However, because the system parses any file not in `_imports`,
templates located outside of `_imports` will be rendered as pages. If they're
not linked, its no big deal, but it is something a user should keep in mind.
